...
(() => {
  ...
  function localFromState(state) {
    const out = {};
    const map = {
      routes:'conductor_routes', tickets:'conductor_tickets',
      shiftActive:'conductor_shift_active', conductorName:'conductor_name',
      terminalID:'conductor_terminal_id', orgName:'conductor_org_name',
      ticketFooter:'conductor_ticket_footer', paperWidth:'conductor_paper_width',
      lastTicketNum:'conductor_last_num', operations:'conductor_operations',
      compositions:'conductor_compositions', currentComposition:'conductor_current_composition',
      shiftStartedAt:'conductor_shift_started_at'
    };
    Object.entries(map).forEach(([prop,key]) => {
      if (state[prop] !== undefined) out[key] = state[prop];
    });
    return out;
  }
  ...
  function applyLocalMap(data) {
    ...
    try {
      ...
      for (const [k,v] of Object.entries(data || {})) {
        if (LOCAL_KEYS.includes(k)) localStorage.setItem(k, typeof v === 'string' ? v : JSON.stringify(v));
      }
    } finally {
  ...
  async function getRemote() {
    const r = await fetch(URL + '?id=eq.' + encodeURIComponent(ROW) + '&select=data,updated_at', { headers });
  ...
  async function putRemote(state) {
    const r = await fetch(URL, {
      ...
      body: JSON.stringify({ id: ROW, data: state, updated_at: new Date().toISOString() })
    });
  ...
  async function startup() {
    try {
      ...
      const localState = stateFromLocal();
      if (remoteKeys) {
  ...
  async function poll() {
    if (busy || applyingRemote) return;
    try {
      const localNow = JSON.stringify(readLocal());
      if (lastLocal && localNow !== lastLocal) {
        await saveLocalToCloud();
        return;
      }
      const row = await getRemote();
      ...
      if (!lastRemote) lastRemote = remoteText;
      if (remoteText !== lastRemote) {
        const currentLocal = JSON.stringify(readLocal());
        const knownLocal = lastLocal || currentLocal;
        if (currentLocal === knownLocal) {
          applyLocalMap(localFromState(remote));
      ...
      console.error('Cloud sync poll:', e);
    }
  }
  ...
  window.addEventListener('beforeunload', () => { if (!applyingRemote) saveLocalToCloud(); });
  ...
  function start() {
    ...
    setInterval(() => {
      ...
      if (lastLocal && now !== lastLocal) saveLocalToCloud();
    }, 700);
  }
